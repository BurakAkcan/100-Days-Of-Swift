...
//        }
        ...
        //Okurken yukarıdaki NSKeyedUnArchiver yerine
        let defaults = UserDefaults.standard
        if let savedPeople = defaults.object(forKey: "people") as? Data {//Data json şeklinde gelir bize
...
//Collectionları yan yana dizmek için kullandığımız kod bloğu
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        ac.addAction(UIAlertAction(title: "Ok", style: .default, handler: { [weak self,weak ac] _ in
            ...
            person.name = newName
           // self?.save()// save fonk buraya yazıyoruz
            self?.save()
...
//    func save(){
//        //Person çeren bir listeyi userDefaulta kaydettik kaydettik
...
//    }
    ...
    //Bunu şu şekilde de yapabiliriz Userdefault a bir object kaydederken
    //Json olarak verimi kaydettim o yüzden Encoder kullanıyorum
    func save(){
        let jsonEncoder = JSONEncoder()
        if let savedData = try? jsonEncoder.encode(people){ //listemi json verisine dönüştürdüm
            let defaults = UserDefaults.standard
            print(String(data: savedData, encoding: .utf8)!)
            defaults.set(savedData, forKey: "people")
        }else{
           print("Fail to save")
        }
...
//    }
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        people.append(person)
       //save()// Save fonk buraya yazıyoruz
        save()
        collectionView.reloadData()
        dismiss(animated: true)
    }
